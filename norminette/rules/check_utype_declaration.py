...
class CheckUtypeDeclaration(Rule):
    ...
    def run(self, context):
        ...
        while context.check_token(
                i, ["SEMI_COLON"]) is False and i < len(
                context.tokens):
            ...
            if context.check_token(i, "IDENTIFIER") is True:
                ...
                if context.check_token(
                    ...
                    tmp = i - 1
                    while context.check_token(
                            tmp - 1, ["MULT", "BWISE_AND", "LPARENTHESIS"]) is True and context.is_operator(tmp) == False:
                        tmp -= 1
...
#        print (ids, utype, contain_full_def)
        ...
        if is_td or (is_td == False and contain_full_def == False):
            ...
            while tmp > 0:
                ...
                if context.check_token(
                        tmp, "TAB") is True and on_newline == False:
                    tab_error = True
                if context.check_token(
                        tmp, "NEWLINE") is True and can_nl_error == False:
                    context.new_error("NEWLINE_IN_DECL",
                                      context.peek_token(ids[-1][1]))
                    can_nl_error = True
                tmp -= 1
            if tab_error:
