...
class TokensBracketsTest(unittest.TestCase):

  def  test_opening_bracket(self):
    self.assertEqual(Lexer("{").getNextToken().type, "OPENING_BRACKET")
  ...
  def  test_opening_square_bracket(self):
    self.assertEqual(Lexer("[").getNextToken().type, "OPENING_SQUARE_BRACKET")

  def  test_closing_square_bracket(self):
    self.assertEqual(Lexer("]").getNextToken().type, "CLOSING_SQUARE_BRACKET")
